...
const Profile = ({ data}) => {
  ...
  const getWordsBadgeAndTooltip = (wordCount) => {
    ...
    if (wordCount < 1000) {
      ...
      WordtooltipText =
        "Word Wizard will be opened after writing at least 1000 words";
  ...
  const getQsBadgeAndTooltip = (questionsCount) => {
    ...
    if (questionsCount < 5) {
      QsbadgeImage = Query0;
      QstooltipText =
        "Query Master will be opened after writing at least 5 Questions";
  ...
  const getChaptersBadgeAndTooltip = (chaptersCount) => {
    ...
    if (chaptersCount == 0) {
      ...
      ChaptertooltipText =
        "Chapter Champ will be opened after completing at least 1 chapter";
  ...
  const getTimeBadgeAndTooltip = (timeCount) => {
    ...
    if (timeCount.includes("m")) {
      ...
      timetooltipText =
        "Time Tracker will be opened after writing for at least 1 hour";
  ...
  const { WordbadgeImage, WordtooltipText } =
    getWordsBadgeAndTooltip(wordCount);
  const { QsbadgeImage, QstooltipText } = getQsBadgeAndTooltip(questionsCount);
  const { ChapterbadgeImage, ChaptertooltipText } =
    getChaptersBadgeAndTooltip(chaptersCount);
